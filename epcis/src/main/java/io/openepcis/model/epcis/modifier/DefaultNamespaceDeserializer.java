...
public class DefaultNamespaceDeserializer extends JsonDeserializer<Object> {
  ...
  public Object deserialize(final JsonParser jp, final DeserializationContext ctxt) throws IOException {
    ...
    final Optional<ConversionNamespaceContext> nsCtxOpt = ConversionNamespaceContext.fromDeserializationContext(ctxt);
  ...
  private void findNamespace(final String key, final ConversionNamespaceContext nsContext) {
    ...
    final Optional<String> namespaceOpt = EPCIS_DEFAULT_NAMESPACES.entrySet().stream().filter(entry -> entry.getKey().equals(prefix)).map(entry -> (String) entry.getValue()).findFirst();
    namespaceOpt.ifPresent(entry -> nsContext.populateEventNamespaces(namespaceOpt.get(), prefix));
  }

  public DefaultNamespaceDeserializer() {
  }
}
