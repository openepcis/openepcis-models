...
public class CommonExtensionModifier {
    ...
    private static LinkedHashMap<String, Object> elementReader(final List<Object> value, final String parentPrefix, final ConversionNamespaceContext nsContext) {
            ...
            if (obj instanceof Element) {
                ...
                if (valueElement.hasAttributes()) {
                    ...
                    for (int attr = 0; attr < attributes.getLength(); attr++) {
                        ...
                        if (!attributeName.equalsIgnoreCase("xmlns") && !attrNodeName.startsWith("xmlns:") && !attrNodeName.startsWith("xsi:")) {
    ...
    public static Element replacePrefix(final Element valueElement) {
        ...
        final String correctPrefix = EPCIS_DEFAULT_NAMESPACES.entrySet().stream().filter(entry -> valueElement.getNamespaceURI().equals(entry.getValue())).map(Map.Entry::getKey).findFirst().orElse(null);
    ...
    public static String getNamespacePrefix(final QName namespace, final ConversionNamespaceContext nsContext) {
        ...
        final Map<String, String> allNamespaces = nsContext != null ? nsContext.getAllNamespaces() : Collections.emptyMap();
    ...
    private static Object convertByXsiType(final Element element, final String textValue) {
        ...
        try {
            switch (type) {
                ...
                case "integer", "int", "long", "short", "byte", "nonNegativeInteger", "positiveInteger", "nonPositiveInteger", "negativeInteger", "unsignedInt", "unsignedLong", "unsignedShort", "unsignedByte" -> {
                    try {
